...
namespace CACPP {
	...
	bool Registration::login(String^ name, String^ password, bool admin, bool loggedin) {
		for each (User ^ u in this->users) {
			if (u->login == name) {
				...
				String^ a = (gcnew String(password->Copy(password) + ((admin) ? ("+") : ("-"))))->GetHashCode().ToString();///ÒÓÒ ÍÅÎÁÕÎÄÈÌÎ ÕÅØÈÐÎÂÀÍÈÅ
				if (u->hash == a) {
					if (loggedin) {
						this->logF = true;
						this->nameL = name;
	...
	bool Registration::addUser(String^ name, String^ password, bool admin) {
		...
		User^ user = gcnew User();
		user->login = name;
		user->hash = (gcnew String(password->Copy(password) + ((admin) ? ("+") : ("-"))))->GetHashCode().ToString();///ÒÓÒ ÍÅÎÁÕÎÄÈÌÎ ÕÅØÈÐÎÂÀÍÈÅ
		this->users->Add(user);
		return true;
	}
